...
class LowerOrEqualTest extends PHPUnit_Framework_TestCase
{
     ...
     */
    public function testLowerOrEqual($expected, Type $column, $value)
    {
        $condition = new Conditions\LowerOrEqual($column, $value);
    ...
    /**
     * @dataProvider providerTestFieldLowerOrEqualThanField
     */
    public function testFieldLowerOrEqualThanField($expected, $columnLeft, $columnRight)
    {
        $condition = new Conditions\LowerOrEqual($columnLeft, $columnRight);

        $this->assertSame($expected, $condition->toString($this->escaper));
    }

    public function providerTestFieldLowerOrEqualThanField()
    {
        return array(
